    ...
    public function show($id): Response
{
    ...
    {
        $paket = Paketin::with(['expedisi'])->findOrFail($id);

        return Inertia::render('Paket/PaketEdit', [
            'paket' => $paket,
    ...
    {
        $paket = Paketin::findOrFail($id);

        $paket->update([
            'bentuk_paket' => $request->bentuk_paket,
            'jumlah_paket' => $request->jumlah_paket,
            'lokasi_simpan' => $request->lokasi_simpan,
            'status_verifikasi' => $request->status_verifikasi,
        ]);

        return redirect()->route('lihat-paket.index')->with('success', 'Data diupdate');
    }
}