...
import { reactive, computed, nextTick, onBeforeUnmount } from 'vue'
...
const handleScanResult = async (decodedText) => {
    ...
    const match = cleaned.match(/^PAKET-(\d+)$/i)

    if (!match) {
        scanState.error = 'QR code tidak valid. Format yang diharapkan: PAKET-5'
        return
    }
    ...
    await closeScanner()
    window.location.href = route('paket.show', paketId)
}
...
const openScanner = async () => {
    ...
    try {
        ...
        scanState.loading = false
        scanState.error = `Kamera gagal dibuka: ${error?.message || 'Unknown error'}`
    }
}
                ...
                <div class="flex flex-col gap-4 border-b border-[#ebefec] px-4 py-5 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        <button
                            type="button"
                            @click="openScanner"
                            class="inline-flex h-[42px] w-[42px] items-center justify-center rounded-xl border border-[#d9e2dc] bg-white text-[#556b60] shadow-sm transition hover:-translate-y-0.5 hover:bg-[#f7faf8]"
                            title="Scan QR Paket"
                        >
                            <svg
                                class="h-5 w-5"
        ...
        >
            <div class="w-full max-w-lg rounded-3xl bg-white p-5 shadow-xl">
                <div class="mb-4 flex items-center justify-between">
                    <div>
                        <h3 class="text-lg font-semibold text-[#556b60]">Scan QR Paket</h3>
                        <p class="mt-1 text-sm text-[#8a9a91]">
                            Arahkan kamera ke QR code dari email penghuni.
                        </p>
                    </div>
                    ...
                    <button
                        type="button"
                        @click="closeScanner"
                        class="rounded-xl bg-[#edf3ef] px-3 py-2 text-sm font-medium text-[#556b60]"
                    >
                        Tutup
                    </button>
                </div>
                ...
                <div
                    id="qr-reader"
                    class="min-h-[320px] overflow-hidden rounded-2xl border border-[#d8e0da] bg-black"
                ></div>

                <p v-if="scanState.loading" class="mt-3 text-sm text-[#8a9a91]">
                    Membuka kamera...
                </p>

                <p v-if="scanState.error" class="mt-3 text-sm text-rose-500">
                    {{ scanState.error }}
                </p>
            </div>
        </div>
